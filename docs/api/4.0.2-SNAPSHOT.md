...
```
timeBatch(<INT|LONG|TIME|BOOL|DOUBLE> unique.key, <INT|LONG> window.time, <INT|LONG> start.time)
```
...
```
externalTimeBatch(<INT|LONG|TIME|BOOL|DOUBLE> unique.key, <LONG> time.stamp, <INT|LONG> window.time, <INT> start.time, <INT|LONG> time.out, <INT|LONG> replace.time.stamp.with.batch.end.time)
```
...
```
firstTimeBatch(<INT|LONG|TIME|BOOL|DOUBLE> unique.key, <INT|LONG> window.time, <INT|LONG> start.time)
```
...
```
first(<INT|LONG|TIME|BOOL|DOUBLE> unique.key)
```
...
```
time(<INT|LONG|TIME|BOOL|DOUBLE> unique.key, <INT|LONG> window.time)
```
